...
public class GestionSouris {
    ...
    /**
     * Pose un bloc à la position cliquée si c'est un emplacement valide
     * @param event L'événement de clic de souris
     */
    private static void poserBlocSouris(MouseEvent event) {
        // Récupérer les coordonnées du clic
        double mouseX = event.getSceneX();
        double mouseY = event.getSceneY();
        ...
        // Obtenir le meilleur emplacement pour poser un bloc
        Point emplacementAPose = aelor.obtenirMeilleurEmplacementPose((int)terrainX, (int)terrainY);

        if (emplacementAPose == null) {
            return;
        }

        int blocX = emplacementAPose.x;
        int blocY = emplacementAPose.y;

        // Essayer de poser le bloc (inclut maintenant la validation du support)
        if (terrain.poserBloc(blocX, blocY, aelor.getTypeBlocDefaut())) {
            // Rafraîchir l'affichage du terrain
            terrainVue.rafraichirTerrain();
        }
    }
