...
public class Inventaire {
    ...
    // Ajouter items
    public void ajouterTerre() { terre++; }
    public void ajouterHerbe() { herbe++; }
    public void ajouterPierre() { pierre++; }

    // Enlever items
    public boolean enleverTerre() {
        if (terre > 0) { terre--; return true; }
        return false;
    }

    public boolean enleverHerbe() {
        if (herbe > 0) { herbe--; return true; }
        return false;
    }

    public boolean enleverPierre() {
        if (pierre > 0) { pierre--; return true; }
        return false;
    }

    // Changer item sélectionné
    public void changerVers(String item) {
        if (peutUtiliser(item)) {
            this.itemActuel = item;
            System.out.println(getNom(item) + " sélectionné(e)");
        }
    }
    ...
    public boolean peutUtiliser(String item) {
        switch (item) {
            case "terre": return terre > 0;
            case "herbe": return herbe > 0;
            case "pierre": return pierre > 0;
    ...
    public boolean utiliserItemActuel() {
        switch (itemActuel) {
            case "terre": return enleverTerre();
            case "herbe": return enleverHerbe();
            case "pierre": return enleverPierre();
            default: return false;
        }
    }
